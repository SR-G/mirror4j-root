...
public class SimpleXMLDocumentationOutputXSD implements ISimpleXMLDocumentationOutput {
    ...
    public String generate(final Class<?> racine, final SimpleXMLDocumentationEntity entity) throws SimpleXMLDocumentationException {
        ...
        for (final String definition : additionnalXsdDefinitions) {
    ...
    private String getMainElementName(final Class<?> racine) {
        for (final Annotation annotation : racine.getAnnotations()) {
     ...
     *             the simple xml documentation exception {@inheritDoc}
     ...
     */
    @Override
    public String mergeContent(final String newContent, final String sourceFileContent) throws SimpleXMLDocumentationException {
        return null;
    }
     ...
     */
    public void setAdditionnalXsdDefinitions(final Collection<String> additionnalXsdDefinitions) {
    ...
    public String toXSD(final SimpleXMLDocumentationEntity currentEntity, final Set<String> alreadyDisplayed) {
        ...
        while (itEntities.hasNext()) {
            ...
            final SimpleXMLDocumentationEntity entity = currentEntity.getEntities().get(name);
            if (entity.isText()) {
                nbTexts++;
            }
        }
        ...
        /* Enum */
        int nbEnums = 0;
        final StringBuffer sbEnum = new StringBuffer();
        if (currentEntity.getType().isEnum()) {
            ...
            final List<String> enumerations = currentEntity.getType().getEnumeration();
        ...
        /* Element */
        int nbElements = 0;
        final StringBuffer sbElements = new StringBuffer();
        ...
        while (itEntities.hasNext()) {
            ...
            if (entity.isElement()) {
                if (entity.isPrimitive()) {
                    ...
                    sbElements.append("            <xsd:element name=\"" + entity.getBaliseName() + "\" type=\"" + entity.getTypeName()
                            + "\" minOccurs=\"0\" />\n");
                }
                nbElements++;
                ...
                if (entity.isInline()) {
                    if (entity.isPrimitive()) {
                        sbElements.append("            <xsd:element name=\"" + entity.getType().getBaliseName()
                                + "\" minOccurs=\"0\" maxOccurs=\"unbounded\" />\n");
                    } else {
                        sbElements.append("            <xsd:element name=\"" + entity.getType().getBaliseName() + "\" type=\"" + entity.getTypeName()
                                + "\" minOccurs=\"0\" maxOccurs=\"unbounded\" />\n");
                    }
                } else {
        ...
        /* Attributs */
        int nbAttributs = 0;
        final StringBuffer sbAttributes = new StringBuffer();
        itEntities = currentEntity.getEntities().keySet().iterator();
        while (itEntities.hasNext()) {
            final String name = itEntities.next();
            final SimpleXMLDocumentationEntity entity = currentEntity.getEntities().get(name);
            if (entity.isAttribute()) {
                if (entity.getClazz().isEnum()) {
                    sbAttributes.append("         <xsd:attribute name=\"" + entity.getBaliseName() + "\" type=\"" + entity.getTypeName() + "\" "
                            + getXSDRequired(entity) + "/>\n");
                } else {
                    sbAttributes.append("         <xsd:attribute name=\"" + entity.getBaliseName() + "\" type=\"" + entity.getPrimitiveXSD() + "\" "
                            + getXSDRequired(entity) + "/>\n");
                }
                nbAttributs++;
            }
        ...
        while (itEntities.hasNext()) {
            final String name = itEntities.next();
            final SimpleXMLDocumentationEntity subEntity = currentEntity.getEntities().get(name);
